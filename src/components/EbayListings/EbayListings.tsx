...
const EbayListings: React.FC = () => {
  ...
  const evaluateCard = (card: Card): ListingRecommendation | null => {
    ...
    const suggestedPrice = Math.round(card.currentValue * 0.9);
  ...
  const recommendations = useMemo(() => {
    const unsoldCards = showOnlyUnsold ? state.cards.filter((card) => !card.sellDate) : state.cards;

    const filtered =
      filterCategory === 'all' ? unsoldCards : unsoldCards.filter((card) => card.category === filterCategory);
  ...
  const startBulkOperation = useCallback((type: BulkOperation['type'], cardIds: string[]) => {
    ...
    setTimeout(() => {
      setBulkOperations((prev) =>
        prev.map((op) => (op.id === operation.id ? { ...op, status: 'running' as const } : op))
      );
  ...
  return (
    <div className="ebay-listings">
      ...
      <AnimatedWrapper animation="fadeInUp" duration={0.6} delay={0.4}>
        <div className="listings-summary card-glass">
          <div className="summary-stats">
            <motion.div className="summary-stat" whileHover={{ scale: 1.05 }} transition={{ duration: 0.2 }}>
                    ...
                    <span className="operation-type">
                      {operation.type === 'export'
                        ...
                        : operation.type === 'price_update'
                          ? '💰'
                          : operation.type === 'category_update'
                            ? '🏷️'
                            : '📊'}
                    ...
                    <CollapsibleMenu title="AI Pricing Suggestions" icon="🤖">
                      <div className="ai-suggestions">
                        {generateAiPricingSuggestions(rec.card).map((suggestion, idx) => (
                          ...
                          >
                            <div className="suggestion-price">{formatCurrency(suggestion.price)}</div>
                            <div className="suggestion-reason">{suggestion.reason}</div>
                            <div className="suggestion-confidence">{suggestion.confidence}% confidence</div>
                          </motion.div>
                        ))}
                      </div>
                    </CollapsibleMenu>
      ...
      {showBulkExport && <BulkEbayExport cards={state.cards} onClose={() => setShowBulkExport(false)} />}
    </div>
  );
};
