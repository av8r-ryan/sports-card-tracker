...
const ScreenReaderSupport: React.FC<ScreenReaderSupportProps> = ({ children, className = '' }) => {
  ...
  useEffect(() => {
    const announcePageChange = () => {
      ...
      accessibilityManager.announce(`Navigated to ${headingText}. ${pageTitle}`, 'polite');
    };
    ...
    const observer = new MutationObserver((mutations) => {
      mutations.forEach((mutation) => {
        if (mutation.type === 'childList') {
          ...
          const hasMainContent = addedNodes.some(
            (node) => node.nodeType === Node.ELEMENT_NODE && (node as Element).querySelector('h1, [role="heading"]')
          );
...
export const screenReaderUtils = {
  ...
  announceValidationError: (fieldName: string, errorMessage: string) => {
    accessibilityManager.announce(`Validation error for ${fieldName}: ${errorMessage}`, 'assertive');
  },
  ...
  createAccessibleFormField: (
    ...
    const field = document.createElement(type === 'textarea' ? 'textarea' : 'input') as
      | HTMLInputElement
      | HTMLTextAreaElement;
    field.id = fieldId;
