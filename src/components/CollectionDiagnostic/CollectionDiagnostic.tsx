...
const CollectionDiagnostic: React.FC = React.memo(() => {
  ...
  const loadDiagnosticData = async () => {
    try {
      ...
      const orphaned = allCards.filter((card) => card.collectionId && !collectionIds.has(card.collectionId));
      ...
      console.log('Diagnostic Data:', {
        ...
        cardsByCollection: allCards.reduce(
          (acc, card) => {
  ...
  const moveAllCardsToDefault = async (fromCollectionId: string) => {
    ...
    if (
      !window.confirm(`Move all ${collectionCards.length} cards from "${collection?.name}" to your default collection?`)
    ) {
      return;
    }
                ...
                <tr key={collection.id}>
                  ...
                  <td>
                    {cardCount > 0 && !collection.isDefault && (
                      <button onClick={() => moveAllCardsToDefault(collection.id)} className="action-btn">
