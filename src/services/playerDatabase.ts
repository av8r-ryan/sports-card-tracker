...
export const PLAYER_DATABASE: Record<string, PlayerInfo[]> = {
  ...
  Basketball: [
    ...
    },
    {
      name: 'Kevin Durant',
      sport: 'Basketball',
      teams: ['SuperSonics', 'Thunder', 'Warriors', 'Nets', 'Suns'],
      years: ['2007-2024'],
      position: 'F',
      nicknames: ['KD', 'Slim Reaper'],
    },
    {
      name: 'Luka Dončić',
      sport: 'Basketball',
    ...
    // Legends
    {
      ...
      years: ['1984-2003'],
      position: 'G',
      nicknames: ['MJ', 'Air Jordan', 'His Airness'],
    },
    {
      name: 'Kobe Bryant',
      sport: 'Basketball',
    ...
    },
    {
      ...
      years: ['1992-2011'],
      position: 'C',
      nicknames: ['Shaq', 'The Big Aristotle', 'Diesel'],
    },
    {
      name: 'Tim Duncan',
      sport: 'Basketball',
  ...
  Football: [
    ...
    { name: 'Barry Sanders', sport: 'Football', teams: ['Lions'], years: ['1989-1998'], position: 'RB' },
    {
      name: 'Brett Favre',
      sport: 'Football',
      teams: ['Falcons', 'Packers', 'Jets', 'Vikings'],
      years: ['1991-2010'],
      position: 'QB',
    },
  ],
...
export const CARD_SET_PATTERNS: Record<string, { sport: string; keywords: string[] }[]> = {
  Topps: [
    {
      sport: 'Baseball',
      keywords: ['Chrome', 'Update', 'Series 1', 'Series 2', 'Heritage', 'Stadium Club', 'Finest', 'Archives'],
    },
    { sport: 'Football', keywords: ['Chrome', 'Finest'] },
  ],
  Bowman: [{ sport: 'Baseball', keywords: ['Chrome', 'Draft', 'Best', 'Sterling', 'Inception', '1st'] }],
  Panini: [
    {
      sport: 'Basketball',
      keywords: ['Prizm', 'Select', 'Mosaic', 'Optic', 'Chronicles', 'National Treasures', 'Immaculate', 'Flawless'],
    },
    {
      sport: 'Football',
      keywords: ['Prizm', 'Select', 'Mosaic', 'Optic', 'Chronicles', 'National Treasures', 'Immaculate', 'Contenders'],
    },
  ],
  'Upper Deck': [
  ...
  Leaf: [{ sport: 'Multi-Sport', keywords: ['Metal', 'Trinity', 'Ultimate', 'Valiant'] }],
};
...
export class PlayerDatabaseService {
  ...
  validatePlayerTeam(playerName: string, teamName: string): boolean {
    ...
    return player.teams.some((t) => t.toUpperCase() === team.name.toUpperCase());
  }
  ...
  getSportFromBrand(brand: string, text: string): string | null {
    ...
    for (const pattern of patterns) {
      const hasKeyword = pattern.keywords.some((keyword) => upperText.includes(keyword.toUpperCase()));
      if (hasKeyword) {
