...
class SportsCardDatabase extends Dexie {
  ...
  constructor() {
    ...
    this.version(2)
      .stores({
        cards:
          'id, userId, player, year, category, createdAt, currentValue, [year+player], [userId+year], [userId+category]',
      })
      .upgrade((trans) => {
    ...
    this.version(3)
      .stores({
        cards:
          'id, userId, collectionId, player, year, category, createdAt, currentValue, [year+player], [userId+year], [userId+category], [userId+collectionId]',
      })
      .upgrade(async (trans) => {
...
export const cardDatabase = {
  ...
  async getAllCards(collectionId?: string): Promise<Card[]> {
    try {
      ...
      const filteredCards = collectionId ? cards.filter((card) => card.collectionId === collectionId) : cards;
      ...
      return processedCards.map((card) => ({
        ...card,
        purchaseDate: typeof card.purchaseDate === 'string' ? new Date(card.purchaseDate) : card.purchaseDate,
        sellDate: card.sellDate
          ? typeof card.sellDate === 'string'
